...
const Technology = () => {
    ...
    return (
        <section className='inner-container space-y-4 pt-4 pb-8 lg:pb-14 bg-bandTernary relative '>
            ...
            <Marquee
                autoFill={true}>
                {skills?.map((skill, i) => <div className='w-12 h-12 lg:w-20 lg:h-20 flex justify-center items-center mx-4 rounded-3xl' key={i}>
                ...
                <div className='col-span-1 flex flex-col justify-center items-center gap-8'>
                    {["Frontend", "Backend", "Database", "Devops"].map((item, index) => (
                        <button
                            ...
                            key={index}
                            className={`text-2xl font-bold bg-bandPrimary py-5 w-full rounded-lg ${showedTechnology === item.toLowerCase() ? 'bg-bandYellow text-bandTernary' : 'text-white'}`}
                        >
                            {item}
                        </button>
                    ))}
                                                    ...
                                                    technology[key].map((item, index) => {
                                                        return (
                                                            <div
                                                                ...
                                                                key={index}
                                                            >
                                                                <Image className={item.image === expressImage ? "bg-white p-2 rounded-lg" : ""} src={item.image} width={100} alt="" />
                                                                <h3 className='text-xl font-bold text-bandYellow'>{item.name}</h3>
                                                            </div>
                                                        )
                                                    })
                                                }
