    ...
    return (
        <section className={`${theme === "light" ? "bg-white" : "bg-bandTernary"}`}>
            <div className="bg-bandOrange text-slate-800 pb-14 md:pb-20 pt-24 md:pt-36 text-center space-y-4">
                ...
                <p className="text-xl md:text-2xl">I create stunning and responsive websites tailored to your business needs</p>
            </div>
            <div className="py-14 my-container space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {webDevelopmentServicesData.map(({ title, description, image }, index) => (
                        <div
                            key={index}
                            className={`${theme === "dark" ? "bg-white" : "bg-slate-400"} p-6 rounded-lg hover:shadow-lg transition duration-300 transform hover:scale-105 shadow-2xl border-2 border-band-orange`}
                        >
